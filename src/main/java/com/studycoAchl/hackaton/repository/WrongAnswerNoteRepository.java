...
public interface WrongAnswerNoteRepository extends JpaRepository<WrongAnswerNote, UUID> {
    ...
    // 기본 조회
    List<WrongAnswerNote> findByUser_Uuid(UUID userUuid);
    List<WrongAnswerNote> findByUser_UuidAndSubject_Uuid(UUID userUuid, UUID subjectUuid);
    ...
    // 키워드별 조회
    List<WrongAnswerNote> findByUser_UuidAndKeyword(UUID userUuid, String keyword);
}